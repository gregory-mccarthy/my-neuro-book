...
<!-- FIGURE TO CREATE. Suggested label: fig-vision-eye-retina. Sagittal eye showing cornea, iris, pupil, lens, vitreous, retina, macula/fovea, optic disc, and optic nerve. Include an enlarged retinal inset with arrows showing light traveling toward the photoreceptors and neural signals traveling back toward ganglion cells. Show the RPE and choroid behind the photoreceptors, and distinguish choroidal from retinal circulation. -->
...
<!-- FIGURE TO CREATE. Suggested label: fig-vision-retinal-layers. Full retinal section with rods, cones, horizontal cells, ON and OFF bipolar cells, representative amacrine cells, and ganglion cells. Name the nuclear and plexiform layers. Use separate arrows for incoming light, vertical neural signaling, and lateral interactions. Bipolar-cell activity should be represented as graded voltage; ganglion-cell output may be represented as spikes. -->
...
<!-- FIGURE TO CREATE. Suggested label: fig-vision-phototransduction. Two panels, DARK and LIGHT INCREMENT, showing one photoreceptor from outer segment to synaptic terminal. Include cGMP, CNG channels, Na+/Ca2+ entry, K+ exit, membrane voltage, voltage-gated Ca2+ channels, and glutamate release. Beneath the light panel, show the opsin → transducin → PDE6 → lower cGMP cascade. Keep darkness on the left in every later figure. -->

### Glutamate does not carry one fixed instruction

Glutamate is often introduced as the principal excitatory transmitter of the adult central nervous system. That is a useful generalization, but it is not a law assigning glutamate one permanent postsynaptic meaning. A neurotransmitter binds to receptors. The receptors, their associated ion channels, and the electrical state of the receiving cell determine the effect.
...
<!-- FIGURE TO CREATE. Suggested label: fig-vision-on-off-bipolar. A cone contacting an OFF bipolar cell with AMPA/kainate receptors and an ON bipolar cell with the mGluR6–Go–TRPM1 cascade. Show DARK on the left and LIGHT INCREMENT on the right. Label glutamate high/low, receptor state, bipolar membrane response, and “sign-preserving” versus “sign-inverting.” A plus or minus sign should indicate net postsynaptic effect, not a permanent property of glutamate. -->
...
<!-- FIGURE TO CREATE. Suggested label: fig-vision-center-surround-computation. A one-dimensional luminance profile containing a uniform dark region, a sharp boundary, and a uniform bright region. Move a small center and broader surround sampling window across it. Beneath the profile, plot the idealized output of center − weighted surround: little sustained output within uniform regions and oppositely signed responses near the boundary. Include the two numerical examples from the text. -->
