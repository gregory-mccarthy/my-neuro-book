...
![**Cone contrast and early opponent channels.** Cone signals are first expressed relative to an adapted background as cone contrasts: $c_\mathrm{L} = (L - L_0)/L_0$, $c_\mathrm{M} = (M - M_0)/M_0$, and $c_\mathrm{S} = (S - S_0)/S_0$. These normalized inputs are then combined schematically to form three post-receptoral channels: a luminance-related achromatic signal, $\approx w_\mathrm{L} c_\mathrm{L} + w_\mathrm{M} c_\mathrm{M}$; an $L$-versus-$M$ signal, $\approx a c_\mathrm{L} - b c_\mathrm{M}$; and an $S$-versus-$(L+M)$ signal, $\approx c c_\mathrm{S} - (d c_\mathrm{L} + e c_\mathrm{M})$. The weights indicate that biological circuits need not combine cone classes equally. Worked examples show the expected pattern of responses for three simplified changes from a neutral gray background: a broadband increment, which strongly drives the achromatic channel while leaving opponent signals relatively small; an $L$-biased change, which shifts the $L$-versus-$M$ channel toward the $L$-dominant side; and an $S$-biased change, which shifts the $S$-versus-$(L+M)$ channel toward the $S$-dominant side. The figure is schematic and emphasizes that early retinal channels encode relations among cone signals rather than directly identifyingwavelength or color.](/unit_4/figs/cone-contrast.jpeg){#fig-vision01-cone-contrast}
