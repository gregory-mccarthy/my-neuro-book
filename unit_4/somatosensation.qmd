...
# Somatosensation: The Body at the Boundary {#sec-somatosensation}
...
![**Anterolateral system for pain, temperature, and itch**. The pathway carrying nociceptive, thermal, crude-touch, and itch information from the body, complementing the dorsal column–medial lemniscus pathway. Free nerve endings in
the skin (nociceptors and thermoreceptors), served by small-diameter Aδ and C fibers, are the peripheral terminals of first-order afferents whose cell bodies lie in the dorsal root ganglion. In contrast to the DCML pathway, these first-order axons synapse immediately in the spinal dorsal horn (laminae I, II, and V), where the second-order cell bodies reside. Second-order axons decussate within the spinal cord via the anterior white commissure and ascend contralaterally in the anterolateral quadrant as the spinothalamic tract. The pathway then diverges into functional streams, each with its own thalamic relay: a sensory-discriminative stream (via VPL to S1) encoding stimulus location and intensity; an interoceptive stream (via VMpo to the dorsal posterior insula) encoding the felt bodily state of the stimulus; and an affective stream (via medial and intralaminar thalamic nuclei to the anterior cingulate cortex)
encoding its unpleasantness. The insular and cingulate projections arise from separate thalamic relays rather than a shared one. Only the S1 route concerns us here; the interoceptive and affective streams, along with a further pathway that reaches the amygdala without passing through the thalamus at all, are taken up in @sec-pain. Because decussation occurs in the cord, a spinal lesion produces contralateral loss of pain and temperature beginning one or two segments below the lesion, in contrast to the ipsilateral loss of fine touch that follows a dorsal-column lesion.](/unit_4/figs/anterolateral-partial.svg){#fig-somatosensation-anterolateral-partial}
...
![**Figure legend.** The stretch reflex arc of the biceps, shown as anatomical compartments. Read the pathway from right to left. A tendon tap (bottom right, dashed line) stretches the biceps and, with it, the muscle spindle embedded in the muscle. Stretch of the spindle generates action potentials in the Ia afferent (blue), whose axon ascends toward the spinal cord. The cell body of this afferent lies in the dorsal root ganglion (DRG), shown as a blue circle on the pathway. Because the afferent is pseudounipolar, the signal passes the soma without an intervening synapse and continues through the dorsal root — labeled *no synapse* — into the dorsal gray of the spinal cord. Within the dorsal gray the afferent bifurcates (small blue node). The two branches take different routes: **The monosynaptic branch** descends directly to the ventral gray, where it makes an excitatory synapse (+, green) onto an alpha motor neuron (red circle). Excitation of this motor neuron drives contraction of the biceps — the muscle that was stretched. **The collateral branch** synapses in the intermediate gray, exciting an inhibitory interneuron (+, green; green circle). This interneuron in turn projects to a second alpha motor neuron in the ventral gray and makes an inhibitory synapse (−) onto it. This second motor neuron innervates the triceps, the antagonist. Its output is suppressed, shown as a dashed red axon. Both alpha motor axons (red) leave the ventral gray, exit the cord through the ventral root, and travel to their target muscles. The net result is contraction of the biceps with simultaneous relaxation of the triceps — reciprocal inhibition — so that the elbow flexes without the antagonist opposing the movement. Note that the Ia afferent is excitatory at *both* of its synapses; the inhibitory sign in this circuit is introduced by the interneuron, not by the afferent itself.](/unit_4/figs/stretch-reflex.svg){#fig-somatosensation-stretch-reflex}
...
This is the first agnosia in this book, and the structure of the argument matters because it recurs. A single well-characterized patient is not weak evidence when the dissociation is sharp and the alternatives have been tested rather than assumed. Recognition disorders in vision are more dramatic and better known, and @sec-visual-agnosia takes them up in detail. The logic that makes them interpretable is the logic set out here.


## How fixed is the map?

The somatosensory homunculus invites a natural inference: a body map this orderly must be permanently hard-wired. A long tradition of research instead described adult cortical maps as highly plastic, and for several decades that account—deprivation frees cortical territory, neighboring inputs expand into it, and phantom sensations reveal the perceptual consequence—became the standard demonstration that adult cortex reorganizes dramatically after injury. More recent work, using stronger designs, has constrained that account substantially. The disagreement is not simply whether cortex can change, but what kind of change should count as a reorganized map—and the two bodies of evidence are not equally weighty.
