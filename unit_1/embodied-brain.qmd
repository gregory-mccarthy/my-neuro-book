...
We ended the last chapter by noting that a patterned neural tube is a controllable tube. The neural tube is not a uniform casing of cells; it is carved by regulatory genes into territories with addresses, and those addresses are the beginnings of structures that will do different jobs. That is where we left the brain — built, regionalized, ready.

But ready to do *what*, though? This is the question that we will take up in this chapter. If you open most textbooks at this point, the next move is a grand tour of the brain's parts: here is the thalamus, here is the cerebellum, here are the four lobes, please memorize the gyri. We will take that tour in @sec-connected-map. However, before we walk the rooms of the house, I want to deal with something that the room-by-room tour tends to skip over, and that I think is actually prior to it.

![The brain embedded in body-regulating loops.](/unit_1/figs/fig-embodied.svg){#fig-embodied}
...
This chapter is therefore a hinge. It is not yet the full anatomy tour. That comes next. It is also not the sensory-systems unit, where we will spend serious time on vision, audition, touch, taste, smell, vestibular signals, and the cortical machinery that builds a world from them. And it is not the later hypothalamus chapter, where we will open the regulatory machinery in detail. The goal here is simpler and more foundational: to show how the brain is coupled to the body it regulates. The nervous system has a peripheral half; the body sends signals upward; the brain sends signals downward; and the loop itself is the unit of regulation.
...
This chapter uses directional language immediately, so here is the minimum vocabulary. We do not need the full anatomical tour yet — that is the next chapter — but we do need to be able to point.

**Afferent** means carrying signals *toward* the central nervous system (body → brain); **efferent** means carrying them *away* from it (brain → body). A useful mnemonic: **A**fferent = **A**rriving, **E**fferent = **E**xiting.

**Directional terms.** *Rostral* means toward the beak or front, and *caudal* means toward the tail or back. *Dorsal* means back, and *ventral* means belly. *Medial* means toward the midline, and *lateral* means toward the edge. In the spinal cord, the **dorsal** side is sensory (signals arriving) and the **ventral** side is motor (signals exiting) — a division that, as we saw last chapter, traces straight back to the alar and basal plates of the neural tube.
...
## Deeper Dive: A preview of an endocrine control loop

The hypothalamus will eventually deserve a chapter of its own, so do not try to memorize this diagram now. It is here only as a preview of the kind of body-regulating loop we will return to later. The hypothalamus can act through nerves, but it can also act through hormones. In the stress axis, hypothalamic signals reach the pituitary, pituitary hormones reach the adrenal cortex, and adrenal hormones feed back on the brain and pituitary.
...
flowchart TD
    ...
    %% The Styling
    style Hypo fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
    style Pit fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
    style Adr fill:#fff3e0,stroke:#e65100,stroke-width:2px
    style Organs fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    style Brain fill:#f5f5f5,stroke:#9e9e9e,stroke-dasharray: 5 5
```

This is not a detour into stress yet. The point is the architecture: signal out, effect in the body, feedback back to the controller. That is the chapter in miniature.
:::
...
That somatic loop is important, but this chapter will not become the sensory-systems unit. Later, when we study touch, pain, vision, hearing, smell, taste, and balance, we will ask how nervous systems build a model of the outside world. Here I want the other half of the story: how the nervous system senses and regulates the body from within.
...
::: {.callout-note}
## Why I keep saying "bidirectional"
...
::: {.callout-note collapse="true"}
## Deeper Dive: The gut-brain axis, true facts, and tempting overreach

The enteric nervous system is genuinely strange, and it is also the locus of some of the most overhyped claims in modern neuroscience. So let me try to draw the honest line, because this is precisely the kind of live, half-settled frontier where your instructor's job is not to give you a tidy answer but to show you where the solid ground ends.

A fact students enjoy, and which is true: a large majority of the body's serotonin (often quoted as ~90%) and a substantial fraction of its dopamine are found in the gut, not the brain. This is real. What it does *not* license is the leap you will see in popular writing — that because "the happy chemical" is mostly in your gut, your gut controls your mood. Gut serotonin and brain serotonin are, for most purposes, separate pools; serotonin does not freely cross the blood–brain barrier, and enteric serotonin is mostly doing local jobs — driving motility, signaling to sensory fibers — that have nothing to do with how you feel on a Tuesday. The fact is true and the inference is mostly false, and I want you to get used to holding those two things apart, because the gut–brain literature is *full* of true facts attached to overreaching inferences.
...
::: {.callout-note collapse="true"}
## Deeper Dive: Measuring autonomic state — psychophysiology
...
The important point for now is not to memorize the NTS, parabrachial nucleus, and hypothalamus as disconnected names. The important point is that the body has a main sensory entrance into the brainstem, and that entrance points toward the regulatory machinery. In the next chapter, when we tour the CNS, the brainstem will no longer be a vague stalk underneath the brain. It will be the first central station for the body's reports.

::: {.callout-note collapse="true"}
## Deeper Dive: The NTS is old, and that is a clue
