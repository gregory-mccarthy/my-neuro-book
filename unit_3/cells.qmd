...
In the diagrams below, **→** means excitation and **⊣** means inhibition. **I** denotes an inhibitory interneuron or inhibitory population.

| Motif | Simple diagram | What it does |
|---|---|---|
| **Feedforward excitation** | A → B → C | Carries excitation forward from one processing stage to the next |
| **Feedforward inhibition** | A → B; A → I ⊣ B | Lets an input excite a target while also recruiting inhibition, often narrowing the time window during which the target remains active |
| **Feedback / recurrent inhibition** | B → I ⊣ B | Lets an active neuron or population recruit inhibition that feeds back to stabilize its own activity |
| **Lateral inhibition** | A → B₁; A → I ⊣ B₂ | Lets an active pathway suppress a neighboring pathway, sharpening contrast between competing representations |
| **Recurrent excitation** | A → B; B → A | Creates a positive-feedback loop that can amplify activity or help sustain a persistent state; inhibition is normally required to keep the loop stable |
| **Disinhibition** | A ⊣ I ⊣ B | Inhibits an inhibitor, thereby releasing a target from inhibition; this is a common mechanism for gating activity |
| **Convergence** | A → D; B → D; C → D | Combines signals from multiple inputs onto one neuron or population |
| **Divergence** | A → B; A → C; A → D | Distributes one signal to multiple downstream targets |These motifs are not tiny machines with only one use. They are patterns that can be reused in different anatomical settings. **Feedforward inhibition** will appear in sensory pathways where timing matters. **Lateral inhibition** will appear when the nervous system sharpens spatial contrast, as in the retina and somatosensory system. **Disinhibition** will appear in cortical microcircuits and basal ganglia loops. **Recurrent excitation** will appear whenever we ask how activity can persist long enough to guide behavior, and recurrent inhibition will appear whenever we ask why that activity does not simply explode.

A small set of cellular circuit motifs recurs throughout the nervous system. The retina, cochlea, thalamus, cortex, and spinal cord are not just chains of anatomical relays; they are circuits built from excitation, inhibition, convergence, divergence, feedback, and modulation. Learning these motifs provides a vocabulary for recognizing them later, when they are embedded in real sensory anatomy.

## Glial cells

We have spent most of the chapter on neurons because neurons provide the fast excitable signaling that the next chapters will analyze. Roughly half the cells in the brain, however, are glial cells. Glia generally do not communicate through the rapid action potentials characteristic of neurons, but they perform essential roles in maintaining the neural environment, supporting metabolism, regulating synaptic function, and shaping neural signaling.
